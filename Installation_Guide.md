...
The V-Flow Fume Extraction System is specifically designed for Voron 2.4 3D printer, offering an effective solution for managing the fumes generated by ABS/ASA materials. This system facilitates the redirection of fumes outside through a window by utilizing a 4" dryer window vent. Engineered for ease of assembly, all components of the V-Flow system can be printed without the need for supports, streamlining the setup process for the user.
...
- **Combined_Exhaust_Vent_System**: Dedicated to components of the Combined Exhaust Ventilation System. 
  - `CEVS-Hose_Adapter-x3.stl`
  - `CEVS-Main_Hub.stl`
  - `Window_Vent_Hose_Adapter.stl` **(Note: This file is required for both single and dual printer setups.)**
- **Printer_Components**: Includes STL files specifically designed for modifications directly on the Voron printers.
...
### Future Enhancements
- **Z Chain Wire Grommet**: An upcoming addition to the `STL_FILES` subfolder  `Accessories_&_Tools` will seal the large hole at the back of the machine. Until then, taping this hole is recommended, similar to sealing bottom panel gaps, for improved environmental control.
...
6. **Fan Interface Ring and Hose Adapter Assembly**:
   - Start by integrating 8 6x3mm magnets into both the `Voron-Fan_Interface_Ring.stl` and `Voron-Hose_Adapter_Ring.stl`. It's crucial to ensure all magnets are oriented with the correct polarization for a strong connection. Use the `Magnet_Guide.stl` from the `Accessories_&_Tools` folder within `STL_FILES` for accurate placement. ![](Reference_Images/Fan_Interface_Ring-Hose_Adapter_Magnets.jpg)
...
## Combined Exhaust Ventilation System (CEVS) Installation Guide
  - The V-Flow system, designed for dual printer support, is a dynamic project with the flexibility to scale. This guide details the current setup for the CEVS, however there is always potential for upgrades or new components to extend its capacity based on need/demand.

### Installation Overview
  - The CEVS is engineered for straightforward installation, with the most challenging aspect being the printing of the `CEVS-Main_Hub.stl` file. This component is pre-oriented to print without the need for supports, though a brim of at least 10mm is recommended for optimal bed adhesion. The same print settings and recommendations as previously mentioned apply here.
